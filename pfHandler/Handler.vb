...
Public Class pfHandler : Inherits iHandler : Implements xmlHandler
    ...
    Public Overrides Sub StreamHandler(ByRef w As XmlTextWriter, ByRef Request As StreamReader)
        ...
        With o
            ...
            If Not .order.payment Is Nothing Then _
                NewPayment(o)
    ...
    Private Function NewCustomer(contact As requestCustomerContact, address As requestCustomerAddress) As String
        ...
        Dim custname As String = ExecuteScalar(
            String.Format(
                "USE [{1}]; SELECT dbo.sp_CustByEmail('{0}')",
                contact.email,
                Me.requestEnv
            )
        )
    ...
    Private Sub NewOrder(ByRef r As request)

        With r
            log.LogData.AppendFormat("Creating order {0} for customer {1}.", .order.orderref, .customer.name, .customer.desc).AppendLine()

            Using ord As New orderLoading
                Dim o As priRow = ord.AddRow("999999", .order.orderref)
                ...
                If Not TypeOf ex Is apiResponse Then Throw (ex)
                With TryCast(ex, apiResponse)
                    log.LogData.AppendFormat("{0}: {1}", .response, .message).AppendLine()

                    For Each msg As apiError In .msgs
                        log.LogData.AppendFormat("  Ln {0}: {1}", msg.Line, msg.message).AppendLine()
                    Next

                    With .msgs(0)
                        If .Loaded Then
                            r.order.PriorityOrdName = .resultKeys("ORDNAME")
                        Else
                            Throw New Exception(.message)
                        End If
    ...
    Private Sub NewPayment(ByRef r As request)

        With r
            log.LogData.AppendFormat("Creating payment for order #{0}.", .order.PriorityOrdName).AppendLine()

            Using pay As New paymentLoading
                Dim p As priRow = pay.AddRow(.customer.name, .order.PriorityOrdName)
                With .order.payment
